...
/**
 * 시험 생성 (강사용)
 * @param {number} courseId - 생성할 강의 ID
 * @param {{ title: string, description: string, start_time: string, end_time: string }} examData - 생성할 시험 정보 객체
 * @returns {Promise<ExamDTO>>
 */
 ...
 * @param {{ title: string, description: string, start_time: string, end_time: string, status: 'PREPARING' | 'CANCELLED' | 'IN_PROGRESS' | 'COMPLETED' }} examData - 수정할 시험 정보 객체
 * @returns {Promise<ExamDTO>>
 */
